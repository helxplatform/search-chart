...
![Version: 8.0.1](https://img.shields.io/badge/Version-8.0.1-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: v3.0.0](https://img.shields.io/badge/AppVersion-v3.0.0-informational?style=flat-square) 
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| dug-lakefs-event-listeners | object | `{"airflow":{"metadataSecret":{"key":"connection","name":"search-metadata"}},"dags":{"default":"annotate_and_index","map":{}},"enabled":false,"hook":{"token":""}}` | lakeFS -> Airflow event listener (subchart, OCI) |

### Elasticsearch

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| elasticsearch | object | `{"clusterHealthCheckParams":"wait_for_status=green&timeout=1s","enabled":true,"esJavaOpts":"-Xmx6g -Xms6g -Dlog4j2.disable.jmx=true -Dlog4j2.formatMsgNoLookups=true","extraEnvs":[{"name":"ELASTIC_PASSWORD","valueFrom":{"secretKeyRef":{"key":"password","name":"search-elastic-secret"}}},{"name":"ELASTIC_USERNAME","valueFrom":{"secretKeyRef":{"key":"username","name":"search-elastic-secret"}}},{"name":"LOG4J_FORMAT_MSG_NO_LOOKUPS","value":"true"}],"image":"docker.elastic.co/elasticsearch/elasticsearch-wolfi","imageTag":"8.19.20","maxUnavailable":0,"replicas":3,"resources":{"limits":{"cpu":1,"ephemeral-storage":"256Mi","memory":"8G"},"requests":{"cpu":1,"ephemeral-storage":"256Mi","memory":"8G"}},"secret":{"enabled":false},"sysctlInitContainer":{"enabled":false}}` | Elastic search for more information checkout full elasticsearch chart options at [https://github.com/elastic/helm-charts/tree/v8.5.1](https://github.com/elastic/helm-charts/tree/v8.5.1) |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| redis | object | `{"auth":{"existingSecret":"search-redis-secret","existingSecretPasswordKey":"password"},"clusterDomain":"cluster.local","commonConfiguration":"# No AOF; RDB snapshots only (the graph is rebuildable from the bulk\n# load csvs in lakefs, so this is a convenience, not the source of truth)\nappendonly no\nsave 300 100000\n# A failed snapshot must not halt the graph. The RDB target is\n# master.persistence.size and a full graph does not necessarily\n# compress under it; with the default `yes`, one failed bgsave makes\n# redis refuse all writes and a bulk load dies with an error that\n# looks nothing like a disk problem.\nstop-writes-on-bgsave-error no\n# Cap below the pod memory limit so redis evicts rather than being\n# OOMKilled. volatile-lru only evicts keys carrying a TTL: the dug\n# annotation response cache sets one (annotation.http_cache_expire_seconds),\n# the falkordb graph keys do not, so the cache is spent first and the\n# graph is never touched. Leave headroom under the limit for\n# fragmentation and bgsave fork copy-on-write.\nmaxmemory 55gb\nmaxmemory-policy volatile-lru","enabled":true,"fullnameOverride":"","image":{"registry":"containers.renci.org","repository":"helxplatform/falkor","tag":"v4.14.4-alpine"},"master":{"command":"","extraFlags":["--loadmodule","/var/lib/falkordb/bin/falkordb.so","THREAD_COUNT","8","OMP_THREAD_COUNT","16"],"livenessProbe":{"enabled":false},"persistence":{"size":"60Gi"},"readinessProbe":{"enabled":true,"failureThreshold":1,"periodSeconds":1},"resources":{"limits":{"cpu":2,"memory":"60Gi"},"requests":{"cpu":1,"memory":"10Gi"}}},"replica":{"autoscaling":{"enabled":false,"maxReplicas":1,"minReplicas":1,"targetCPU":100},"extraFlags":["--loadmodule","/FalkorDB/bin/src/falkordb.so","THREAD_COUNT","8","OMP_THREAD_COUNT","16"],"livenessProbe":{"enabled":false},"persistence":{"size":"50G"},"readinessProbe":{"enabled":true,"failureThreshold":1,"periodSeconds":1,"successThreshold":5},"replicaCount":0,"resources":{"limits":{"cpu":2,"memory":"45Gi"},"requests":{"cpu":1,"memory":"45Gi"}}},"serviceAccount":{"create":false},"usePassword":true}` | Redis (Redis Graph)  for more information checkout full redis  chart options at [https://github.com/bitnami/charts/tree/redis/18.1.1/bitnami/redis](https://github.com/bitnami/charts/tree/redis/18.1.1/bitnami/redis) |
...
| airflow.config.core.parallelism | int | `12` |  |
| airflow.config.kubernetes_executor.task_publish_max_retries | int | `-1` |  |
| airflow.config.workers.max_failed_heartbeats | int | `20` |  |
...
| airflow.postgresql.image.tag | string | `"18.4.2"` |  |
| airflow.postgresql.primary.extendedConfiguration | string | `"max_connections = 500\n"` |  |
...
| airflow.scheduler.logGroomerSidecar.args | list | `[]` |  |
| airflow.scheduler.logGroomerSidecar.command[0] | string | `"bash"` |  |
| airflow.scheduler.logGroomerSidecar.command[1] | string | `"-c"` |  |
| airflow.scheduler.logGroomerSidecar.command[2] | string | `"RETENTION=\"${AIRFLOW__LOG_RETENTION_DAYS:-7}\"\nDIR=\"${AIRFLOW__LOGGING__BASE_LOG_FOLDER:-/opt/airflow/logs}\"\nwhile true; do\n  find \"$DIR\" -type f -name \"*.log\" -mtime +\"$RETENTION\" -delete\n  find \"$DIR\" -mindepth 1 -type d -empty -delete\n  sleep 900\ndone\n"` |  |
| airflow.scheduler.logGroomerSidecar.enabled | bool | `true` |  |
| airflow.scheduler.logGroomerSidecar.retentionDays | int | `7` |  |
